...
void main() {
    ...
    float alpha = clamp(dot(col, vec3(0.299, 0.587, 0.114)) * 1.85, 0.0, 1.0);
    alpha *= smoothstep(0.0, 0.38, vUv.x);
...
export default function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ alpha: true, dpr: resolvedDpr, premultipliedAlpha: false });
  ...
  return (
    <div
      ref={containerRef}
      className={cn("relative h-full w-full overflow-hidden bg-transparent [&_canvas]:block [&_canvas]:h-full [&_canvas]:w-full [&_canvas]:bg-transparent", className)}
      style={style}
      {...rest}
    />
  );
