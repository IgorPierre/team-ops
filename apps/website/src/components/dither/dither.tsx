...
import { forwardRef, useEffect, useRef, type ForwardRefExoticComponent, type Ref } from "react";
...
type UniformMap = Map<string, THREE.Uniform<number>>;
...
const RetroEffect = forwardRef<InstanceType<typeof RetroEffectImpl>, { colorNum: number; pixelSize: number }>(
  function RetroEffect({ colorNum, pixelSize }, ref) {
    const Node = WrappedRetro as unknown as ForwardRefExoticComponent<{
      colorNum: number;
      pixelSize: number;
      ref?: Ref<RetroEffectImpl>;
    }>;
    return <Node ref={ref} colorNum={colorNum} pixelSize={pixelSize} />;
  },
);
